...
type XNodeSource = Element | XMLDocument;
type XNodeCallback = string | ((node: XNode) => boolean);
type XNodeNamespaceMap = Record<string, string>;

class XNode {
  private attrs: Record<string, string> = {};
  private name?: string;
  private readonly node: XNodeSource;
  private readonly nsMap: XNodeNamespaceMap;
  private [KEY_CHILDREN]?: XNode[];

  constructor(node: XNodeSource, nsMap: XNodeNamespaceMap = {}) {
    this.node = node;
    this.nsMap = { ...nsMap };
    this.parseAttrs();
    this.parseName();
  ...
  map<T>(callback: (node: XNode) => T) {
    return this.children().map(callback);
  }

  getCallback(callback: XNodeCallback) {
  ...
  filter(callback: XNodeCallback) {
    return this.children().filter(this.getCallback(callback));
  }

  find(callback: XNodeCallback) {
