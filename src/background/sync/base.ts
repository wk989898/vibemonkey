...
import { forEachEntry, objectSet, objectPick, objectGet } from "@/common/object";
...
function serviceConfig(name) {
  function getKeys(key = undefined) {
    const keys = normalizeKeys(key);
...
function objectPurify(obj) {
  ...
  if (Array.isArray(obj)) {
    ...
    forEachEntry.call(obj, ([key, value]) => {
...
events.on("change", (state: { status?: unknown }) => {
...
export const BaseService = serviceFactory({
  ...
  async _refresh() {
    ...
    setSyncState({
      status: result?.code === INIT_UNAUTHORIZED ? SYNC_UNAUTHORIZED : SYNC_AUTHORIZED,
    });
  },
  prepare() {
  ...
  async _request(options) {
    ...
    while (attempts > 0) {
      ...
      try {
        const res = (await request(url, options)) as { data: unknown };
  ...
  async sync() {
    ...
    if (getSyncState().status !== SYNC_AUTHORIZED || getCurrent() !== this.name) return;
  ...
  async _sync() {
    ...
    let remoteChanged =
      !remoteTimestamp || Object.keys(remoteMetaInfo).length !== remoteData.length;
    const now = Date.now();
    ...
    const compareItems = (localItem, remoteItem, remoteInfo) => {
      ...
      if (localItem && remoteItem && remoteInfo) {
        ...
        if (firstSync || !localModified || remoteModified > localModified) return -1;
    ...
    forEachEntry.call(remoteItemMap, ([uri, item]) => {
    ...
    const promiseQueue = [
      ...
      ...updateLocal.map(({ local, info }) => {
        const updates: { props?: { position: number } } = {};
        if (info.position) {
...
function getService(name = undefined) {
