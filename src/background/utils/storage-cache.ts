...
const cachedStorageApiImpl = {
  async get(keys?: string[] | null) {
    ...
    if (!keys || keys.length) {
      ...
      forEachEntry.call(await api.get(keys), ([key, val]) => {
        res[key] = val;
        dbKeys.set(key, 1);
  ...
  async set(data: Record<string, any>, flushNow?: boolean) {
    const toWrite = {};
    const keys = [];
    cache.batch(true);
    forEachEntry.call(data, ([key, val]) => {
    ...
    const toDelete = keys.filter((key) => {
      ...
      if (ok) {
        ...
        if (storage[S_VALUE].toId(key)) {
          ...
          if (key.startsWith(S_SCRIPT_PRE)) updateScriptMap(key, undefined);
...
export const cachedStorageApi = (storage.api =
  cachedStorageApiImpl as typeof browser.storage.local) as typeof cachedStorageApiImpl;
...
function watchStorage(fn, cfg, state = true) {
  ...
  forEachEntry.call(cfg, ([area, ids]) => {
...
async function updateScriptSizeContributor(key, val?: unknown) {
...
function flushLater() {
  if (!flushTimer && !isEmpty(valuesToFlush)) {
    flushTimer = setTimeout(
      ...
      Math.min(
        FLUSH_MAX_DELAY,
        FLUSH_DELAY * Math.max(1, deepSize(valuesToFlush) / FLUSH_SIZE_STEP),
      ),
    );
  }
}
