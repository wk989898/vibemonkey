...
const API_CONFIG: chrome.webRequest.RequestFilter = {
  urls: ["*://*/*"], // `*` scheme matches only http and https
  types: [
    "main_frame" as chrome.webRequest.ResourceType,
    "sub_frame" as chrome.webRequest.ResourceType,
  ],
};
const API_EXTRA = [
  CAN_BLOCK_WEB_REQUESTS && "blocking", // used for xhrInject and to make Firefox fire the event before GetInjected
  kResponseHeaders,
  webRequestEx.OnHeadersReceivedOptions?.EXTRA_HEADERS,
...
const isUnsafeConcat = (s) =>
  (s = s.charCodeAt(s.match(SKIP_COMMENTS_RE)[0].length)) === 45 /*"-"*/ ||
  s === 43 /*"+"*/ ||
  s === 91 /*"["*/ ||
  s === 40 /*"("*/;
...
const META_KEYS_TO_ENSURE_FROM = [[HOMEPAGE_URL, "homepage"]];
...
const resolveDataCodeStr = `(${(global, data) => {
  if (global.vmResolve)
    global.vmResolve(data); // `window` is a const which is inaccessible here
...
const normalizeRealm = (val) => (KNOWN_INJECT_INTO[val] ? val : injectInto || AUTO);
...
const OPT_HANDLERS = {
  ...
  defaultInjectInto(value) {
    ...
    cache.destroy();
    if (injectInto) {
      // already initialized, so we should update the listener
      if (value === CONTENT) {
        API_HEADERS_RECEIVED.removeListener(onHeadersReceived);
      } else if (isApplied && IS_FIREFOX && !xhrInject) {
        API_HEADERS_RECEIVED.addListener(onHeadersReceived as any, API_CONFIG, API_EXTRA as any);
      }
    }
    injectInto = value;
  },
  ...
  [EXPOSE](value) {
    forEachEntry.call(value, ([site, isExposed]) => {
      expose[decodeURIComponent(site)] = isExposed;
    });
  },
};
if (contentScriptsAPI) {
  (OPT_HANDLERS as typeof OPT_HANDLERS & { ffInject?: typeof toggleFastFirefoxInject }).ffInject =
    toggleFastFirefoxInject;
}
...
addPublicCommands({
  ...
  async GetInjected({ url, [FORCE_CONTENT]: forceContent, done }, src) {
    ...
    if (skip > 0) {
      // first time loading the tab after skipScripts was invoked
    ...
    return isApplied ? !done && inject : { [INJECT_INTO]: "off", ...inject };
  },
  async InjectionFeedback(
    { [FORCE_CONTENT]: forceContent, [CONTENT]: items, [MORE]: moreKey, url },
    src,
  ) {
    ...
    const env = cache.get(moreKey) ||
      cache.put(moreKey, getScriptsByURL(url, isTop, undefined, undefined)) || { [SCRIPTS]: [] }; // scripts got removed or the url got blacklisted after GetInjected
...
onStorageChanged((keys: string[], data: Record<string, unknown>) => {
  const toClear = [];
...
function togglePreinject(enable) {
  ...
  if (
    !isApplied /* remove the listener */ ||
    (IS_FIREFOX && !xhrInject && injectInto !== CONTENT) /* add 'nonce' detector */
  ) {
    API_HEADERS_RECEIVED[onOff](onHeadersReceived as any, config, config && (API_EXTRA as any));
  }
...
function toggleFastFirefoxInject(enable) {
  ...
  if (!enable) {
    cache.some((v) => {
      unregisterScriptFF(v); /* must return falsy! */
    });
  } else if (!xhrInject) {
...
function onHeadersReceived(info) {
  ...
  if (bag && !bag[FORCE_CONTENT] && bag[INJECT]?.[SCRIPTS] && !skippedTabs[info.tabId]) {
    const ffReg = IS_FIREFOX && info.url.startsWith("https:") && detectStrictCsp(info, bag);
    if (!CAN_BLOCK_WEB_REQUESTS) {
      return ffReg;
    }
    const res = xhrInject && prepareXhrBlob(info, bag);
    return ffReg ? ffReg.then(() => res) : res;
  }
}
...
function prepare(cacheKey, url, isTop) {
  const shouldExpose = isTop && url.startsWith("https://") ? expose[url.split("/", 3)[2]] : null;
  ...
  const env = getScriptsByURL(url, isTop, errors, undefined);
  const res = env || bagNoOp;
  cache.put(cacheKey, res); // must be called before prepareBag overwrites it synchronously
  if (env) {
    env[PROMISE] = prepareBag(
      cacheKey,
      url,
      isTop,
      env,
      shouldExpose != null ? { [EXPOSE]: shouldExpose } : {},
      errors,
    );
  }
  return res;
...
async function prepareBag(cacheKey, url, isTop, env, inject, errors) {
  ...
  Object.assign(
    ...
    objectPick(env, [S_CACHE, "clipFF", "xhr"]),
  );
  forEachValue.call(propsToClear, (val) => {
...
function prepareScript(script, env) {
  ...
  const injectedCode = [];
  const metaCopy = mapEntry.call(meta, null, pluralizeMeta);
  ...
  for (const url of meta[S_REQUIRE]) {
    const req =
      require[pathMap[url] || url] ||
      `/* ${VIOLENTMONKEY} is missing @require ${url.replace(
        /\*\//g,
        "%2A/",
      )}\n${kTryVacuuming} */`;
    if (/\S/.test(req)) {
      injectedCode.push(
        ...[tmp && isUnsafeConcat(req) && ";", req, !NEWLINE_END_RE.test(req) && "\n"].filter(
          Boolean,
        ),
      );
      tmp = true;
  ...
  injectedCode.push(
    ...[
      ...
      IS_FIREFOX && ";0",
      "\n//# sourceURL=",
      getScriptPrettyUrl(script, displayName),
    ].filter(Boolean),
  );
  return {
    code: "",
...
function triageRealms(scripts, forceContent, tabId, frameId, bag = undefined) {
  let code;
  let wantsPage;
  const toContent = [];
...
function triagePageRealm(env, forceContent = false) {
...
function injectContentRealm(toContent, tabId, frameId) {
  for (const [id, dataKey] of toContent) {
    const scr = cache.get(S_SCRIPT_PRE + id) as VMInjection.Script | undefined; // TODO: recreate if expired?
...
function detectStrictCsp(info, bag) {
  ...
  if (!tmp) return;
  const nonceMatch = tmp.match(NONCE_RE);
  if (nonceMatch) {
    bag[INJECT].nonce = nonceMatch[1];
  } else if (
  ...
  if (m && !nonceMatch) {
    ...
    return Promise.all([m, (bag[CSAPI_REG] = registerScriptDataFF(bag[INJECT], info.url))]).then(
      () => undefined,
    );
  }
}
...
function clearFrameData(tabId = undefined, frameId = undefined, tabRemoved = undefined) {
