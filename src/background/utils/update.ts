...
addOwnCommands({
   ...
   */
  UpdateDeps: (opts) =>
    fetchResources(getScriptById(opts.id), {
      [FETCH_OPTS]: { ...NO_CACHE },
      update: {},
      ...opts,
    }),
...
async function doCheckUpdate(id, script, urls, opts) {
  ...
  try {
    ...
    msgErr = update.error || (!update[kChecking] && (await fetchResources(script, opts)));
    if (process.env.DEBUG) console.error(update);
  } finally {
    if (canNotify(script) && (msgOk || msgErr)) {
      res = {
        script,
        text: trueJoin.call([msgOk, msgErr], "\n"),
        err: !!msgErr,
      };
    }
    delete processes[id];
...
async function downloadUpdate(script, urls, opts) {
  ...
  try {
    if (opts.force) {
      announceUpdate();
      const response = await requestNewer(downloadURL || updateURL, opts);
      return response && "data" in response ? response.data : undefined;
    }
    const response = await requestNewer(updateURL, { ...FAST_CHECK, ...opts });
    ...
    } else if (downloadURL === updateURL && textData.replace(METABLOCK_RE, "").trim()) {
      // Code is present, so this is not a smart server, hence the response is the entire script
      announce(i18n("msgUpdated"));
      return textData;
    } else {
      announceUpdate();
      return downloadURL === updateURL && metaStr.trim() !== textData.trim()
        ? textData
        : (async () => {
            const downloadResponse = await requestNewer(downloadURL, opts);
            return downloadResponse && "data" in downloadResponse
  ...
  function announce(
    ...
    {
      error,
      [kChecking]: checking = !error,
    }: { error?: { status?: number; url?: string } | null; [kChecking]?: boolean } = {},
  ) {
...
function canNotify(script) {
  ...
  return getOption("notifyUpdatesGlobal") ? allowed : (script.config.notifyUpdates ?? allowed);
}
