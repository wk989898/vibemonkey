...
(async () => {
  ...
  forEachEntry.call(data, ([key, script]) => {
...
export function getScriptsByIdsOrAll(ids = undefined) {
...
export function getSizes(ids) {
  ...
  return scripts.map(({ meta, custom: { pathMap = {} }, props: { id } }, i) => [
    ...
    Object.values(meta.resources as StringMap).reduce(getSizeForResources, {
      len: 0,
      pathMap,
    }).len,
  ]);
...
function getSizeForRequires(accum: { len: number; pathMap: StringMap }, url: string) {
  accum.len += (scriptSizes[S_REQUIRE_PRE + (accum.pathMap[url] || url)] || 0) + url.length;
  return accum;
}

function getSizeForResources(accum: { len: number; pathMap: StringMap }, url: string) {
...
export async function removeScripts(ids) {
  ...
  const newLen =
    1 +
    removedScripts.reduce((iAlive, script, i) => {
...
export function checkRemove({ force }: { force?: boolean } = {}) {
  const now = Date.now();
  const ids = removedScripts
    .filter((script) => {
...
export async function parseScript(src) {
  ...
  const result: {
    ...
    where?: { id: number };
    [S_CODE]?: string;
  } = { errors, update: update as VMScript & { message: string } };
  ...
  for (const key of ["config", "custom", "props"]) {
    ...
    forEachEntry.call(src[key], ([srcKey, srcVal]) => {
...
/** @return {Object} */
function buildPathMap(script, base = undefined) {
  const { meta } = script;
  const baseUrl = base || script.custom.lastInstallURL;
  const pathMap = baseUrl
    ? [...meta.require, ...Object.values(meta.resources), meta.icon].reduce((map, key) => {
        if (key) {
...
export async function fetchResources(script, src) {
  ...
  const error = trueJoin.call(errors.map(formatHttpError), "\n");
  if (error) {
    ...
    message += "\n" + error;
    return src.force ? { script, text: message } : message;
  }
}
...
async function fetchResource(src, type, url) {
  let res;
  if (!isRemote(url) || src.update || (await storage[type].getOne(url)) == null) {
    const { portId } = src;
...
function formatHttpError(e) {
  return (e && trueJoin.call([e.status && `HTTP${e.status}`, e.url], " ")) || e;
}
 ...
 */
export async function vacuum(data = undefined) {
  ...
  const noFetch = data && [];
  const sizes = {};
  const result: { errors?: string[]; fixes?: number } = {};
  const toFetch = [];
  const keysToRemove = [];
  /** -1=untouched, 1=touched, 2(+scriptId)=missing */
  const status = {};
  const prefixRe = RegExp(
    `^(${[S_VALUE_PRE, S_CACHE_PRE, S_REQUIRE_PRE, S_CODE_PRE, S_MOD_PRE].join("|")})`,
  );
  const prefixIgnoreMissing = [S_VALUE_PRE, S_MOD_PRE];
  const downloadUrls = {};
  const touch = (prefix, id, scriptId, pathMap = undefined) => {
  ...
  getScriptsByIdsOrAll().forEach((script) => {
    ...
    if (updUrls) {
      const updUrlList = Array.isArray(updUrls) ? updUrls : [updUrls];
      updUrlList.forEach((url) => touch(S_MOD_PRE, url, id));
      downloadUrls[id] = updUrlList[0];
    }
    ...
    forEachValue.call(meta.resources, (url) => touch(S_CACHE_PRE, url, id, pathMap));
    if (isRemote(icon)) touch(S_CACHE_PRE, icon, id, pathMap);
  });
  forEachEntry.call(status, ([key, value]) => {
    if (value < 0) {
      ...
      if (noFetch) {
        ...
        toFetch.push(
          area
            .fetch(url)
            .catch(
              (err) => `${getScriptName(getScriptById(+id || value - 2))}: ${formatHttpError(err)}`,
            ),
        );
      }
    }
