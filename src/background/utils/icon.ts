  ...
  const makeMethod =
    ...
    (...args) => {
      try {
        ...
        fn.call(api, ...args, ignoreChromeErrors);
      } catch (e) {
        fn.call(api, ...args);
      }
    };
  return objectPick(api, ["setIcon", "setBadgeText", "setBadgeBackgroundColor", "setTitle"], (fn) =>
    fn ? makeMethod(fn) : noop,
  );
})();
...
const titleDefault = extensionManifest[BROWSER_ACTION]?.default_title || extensionManifest.name;
const iconDefault = (
  extensionManifest[BROWSER_ACTION]?.default_icon?.[16] || extensionManifest.icons[16]
).match(/\d+(\w*)\./)[1];
...
init.then(async () => {
  ...
  if (contextMenus) {
    const addToIcon = (id, title, opts = undefined) =>
      new Promise((resolve) =>
        contextMenus.create(
          {
          ...
          () => {
            ...
            if (err && !DUPLICATE_MENU_ID_RE.test(err.message || "")) {
              ignoreChromeErrors();
            }
            resolve(undefined);
          },
    ...
    ] as [string, string, typeof badgeChild?][]) {
...
function resetBadgeData(tabId, isInjected = undefined) {
...
function updateState(tab, data, title = undefined) {
  const tabId = tab.id;
...
async function setIcon({ id: tabId }: { id?: number } = {}, data = badges[tabId] || {}) {
  ...
  const payload: {
    ...
    imageData?: Record<number, ImageData | undefined>;
  } = {
    tabId,
    path: pathData,
  };
...
export function getFailureReason(url, data = undefined, def = titleDefault) {
  if (!injectableRe.test(url)) {
    return [titleNoninjectable, INJECT_INTO];
  }
  const blacklistedUrl = testBlacklist(url);
  if (blacklistedUrl) {
    return [titleBlacklisted, "blacklisted", blacklistedUrl];
  }
  if (!isApplied || data?.[INJECT] === "off") {
    return [titleDisabled, IS_APPLIED];
  }
  if (!data) {
    return [];
  }
  return data[INJECT] === SKIP_SCRIPTS ? [titleSkipped, SKIP_SCRIPTS] : [def];
}
...
async function loadIcon(url) {
  if (MV3 && typeof Image === "undefined") {
    const res = url.startsWith(ICON_PREFIX) ? url : await loadDataUri(url);
    iconCache[url] = res;
    return res;
  }
  const img = new Image();
...
async function loadDataUri(url) {
  url = url.startsWith("data:")
    ? url
    : url[0] === "i"
      ? url
      : makeDataUri((url = await loadStorageCache(url))) || url;
...
function bytesToBase64(bytes) {
  ...
  for (let i = 0; i < bytes.length; i += step) {
    res += String.fromCharCode(...bytes.subarray(i, i + step));
  }
  return btoa(res);
}
