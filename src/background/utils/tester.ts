 ...
 */
const cacheMat = initCache({
  lifetime: 60 * 60e3,
});
const cacheInc = initCache({
  lifetime: 60 * 60e3,
});
const cacheResultMat = initCache({
  lifetime: 60e3,
});
const cacheResultInc = initCache({
  lifetime: 60e3,
});
...
export const { reset: resetBlacklist, test: testBlacklist } = blacklist[BLACKLIST];
...
export class MatchTest {
  scheme: string;
  scheme2: string | null;
  host: RegExp | StringTest | null;
  path: RegExp | StringTest | null;
  ...
  test() {
    return (
      (this.scheme === curScheme || this.scheme2 === curScheme) &&
  ...
  static try(rule) {
    ...
    if (parts) {
      const [, scheme, httpMod, host, path = ""] = parts;
      return new MatchTest(rule, scheme, httpMod, host, path);
    }
...
class StringTest {
  ...
  /** @returns {?StringTest} */
  static try(rule, ignoreCase = false) {
...
export function testerBatch(arr = undefined) {
  cacheMat.batch(arr);
  cacheInc.batch(arr);
...
export function testScript(url, script) {
  ...
  const len =
    (matex1 = (custom.origMatch && meta.match) || "").length +
      ...
      for (const rule of rules) {
        ...
        if (res == null) {
          ...
          if ((err = m.err)) {
            if (batchErrors) {
              err = err.message || err;
              err = url ? `${err} - ${scriptUrl || (scriptUrl = getScriptPrettyUrl(script))}` : err;
...
function pathMatcher(tail) {
  ...
  return (
    (hasHash && StringTest.try(tail, false)) ||
  ...
  function reset(value) {
    ...
    for (let text of Array.isArray(value) ? value : (value || "").split("\n")) {
      try {
        ...
        const m =
          ((isInc || mode === "@exclude") && emplace(cacheInc, rule, autoReg)) ||
