...
export function parseMeta(
  ...
  // initialize meta
  const meta = mapEntry.call(metaTypes, (value) => value.default());
...
export function newScript(data = undefined) {
  const state = {
    url: "*://*/*",
    name: "",
    ...data,
  };
  const code = (getOption(kScriptTemplate) || defaults[kScriptTemplate]).replace(
    /{{(\w+)(?::(.+?))?}}/g,
    (str, name, format) =>
      state[name] ??
      (name !== "date" ? str : format ? formatDate(format) : new Date().toLocaleString()),
  );
  const script = {
...
function inferScriptSupportUrl(script, home = getScriptHome(script)) {
  let u =
    home &&
    home.match(
      /^https:\/\/(?:(?:(greas|sleaz)yfork\.(?:org|cc)(?:\/(?!scripts)[^/]+)?|openuserjs\.org)(?=\/scripts\/)|github\.com)\/[^/]+\/[^/]+/,
    );
  if (u) {
 ...
 * @param {boolean} [isIds]
 */
export function updateVisitedTime(arr, isIds = false) {
  const now = Date.now();
  const toBroadcast = {};
  const toWrite = {};
  for (let v of arr) {
