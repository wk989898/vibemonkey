...
addOwnCommands({
  async InitPopup() {
    const tab = (await getActiveTab()) || ({} as Partial<chrome.tabs.Tab>);
    const { url = "", id: tabId = 0 } = tab;
    ...
    if (!failure[0] && badgeData[INJECT] == null) {
      if (!(await isInjectable(tabId, badgeData))) {
        failure = getFailureReason("", badgeData);
...
async function isInjectable(tabId, badgeData) {
  return (
    (badgeData[INJECT] && (await sendTabCmd(tabId, VIOLENTMONKEY, null, { [kFrameId]: 0 }))) ||
    (await executeCode(tabId, "1", { runAt: "document_start" }).catch(() => []))[0]
  );
}
