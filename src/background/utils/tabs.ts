...
const openers = {};
const openerTabIdSupported =
  !IS_FIREFOX || // supported in Chrome
...
export const NEWTAB_URL_RE =
  /^(about:(home|newtab)|(chrome|edge):\/\/(newtab\/|startpageshared\/|vivaldi-webui\/startpage))$/;
/** @returns {string|number} documentId for a pre-rendered top page, frameId otherwise */
export const getFrameDocId = (isTop, docId, frameId) => (isTop === 2 && docId) || frameId;
/** @param {VMMessageSender} src */
export const getFrameDocIdFromSrc = (src) => (src[kTop] === 2 && src[kDocumentId]) || src[kFrameId];
export const getFrameDocIdAsObj = (id) =>
  +id >= 0
    ? {
        [kFrameId]: +id,
      }
 ...
 * @returns {string}
 */
export const getTabUrl = (tab) => tab.pendingUrl || tab.url || "";
...
addOwnCommands({
  ...
  async OpenEditor(pathId, src) {
    return openDashboard(
      `${SCRIPTS}/${pathId || `_new/${src?.tab?.id || (await getActiveTab()).id}`}`,
      src,
    );
  },
...
addPublicCommands({
  ...
  async TabOpen({ url, active = true, container, insert = true, pinned }, src = {}) {
    const isRemoved = src._removed;
    // src.tab may be absent when invoked from popup (e.g. edit/create buttons)
    const srcTab =
      (!isRemoved && src.tab) || (await getActiveTab(isRemoved && src.tab[kWindowId])) || {};
    ...
    if (!/^[-\w]+:/.test(url)) {
      url = isInternal ? browser.runtime.getURL(url) : vetUrl(url, srcUrl);
    }
    if (
      isInternal &&
       ...
       * and a workaround is too convoluted to add it for such an ancient version */ (!storeId ||
        FIREFOX >= 64)
    ) {
      const wndOpts = {
        url,
        incognito: canOpenIncognito && incognito,
        ...(getOption("editorWindowSimple") && {
          type: "popup",
        }),
        ...(!IS_FIREFOX && {
          focused: !!active,
        }),
        // FF doesn't support this
        ...storeId,
      };
    ...
    for (let retry = 0; !newTab && retry < 2; retry++)
      try {
    ...
    if (active && newTab[kWindowId] !== windowId) {
      await browserWindows?.update(newTab[kWindowId], {
        focused: true,
      });
    }
