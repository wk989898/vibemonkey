<template>
  <div>
    <button
      v-text="i18n('buttonImportData')"
      @click="pickBackup"
      ref="buttonImport"
      :disabled="store.batch"
    />
    <button
      v-text="i18n('buttonUndo') + undoTime"
      @click="undoImport"
      class="has-error"
...
async function doImportBackup(file) {
  ...
  const vmEntry = entries.find((entry) =>
    ["vibemonkey", "violentmonkey"].includes(entry.filename?.toLowerCase()),
  );
  ...
  sendCmdDirectly("CheckPosition", null);
  await reader.close();
  reportProgress();
...
function resolveOnUndoMessage(resolve) {
  listenOnce.call(undoPort.onMessage, resolve);
}
