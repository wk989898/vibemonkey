<template>
  <div class="mr-1c">
    ...
    <button
      @click="confirmDanger(resetSettings, i18nResetSettings)"
...
function resetSettings() {
  const ignoredKeys = ["lastModified", "lastUpdate", "sync"];
  const diff = mapEntry.call(
    defaults,
    null,
    (key, defVal) => !ignoredKeys.includes(key) && !deepEqual(defVal, options.get(key)) && key,
...
async function vacuum() {
  await runInBatch(async () => {
    ...
    const { fixes, errors } = (await sendCmdDirectly("Vacuum", null)) as {
      fixes?: number;
      errors?: string[];
    };
