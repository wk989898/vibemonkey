...
import Installed from "./tab-installed.vue";
import Settings from "./tab-settings/index.vue";
...
const current = computed<TabItem>(() => {
  const name = store.route.paths[0];
  return tabs.find((tab) => tab.name === name) || tabs[0];
});

const numbers = computed<Record<string, number>>(() => ({
