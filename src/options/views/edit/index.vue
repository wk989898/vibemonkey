<template>
  <div class="edit frame flex flex-col abs-full" :class="{ frozen }">
    <div class="edit-header flex mr-1c">
      ...
      <div class="mr-1" v-if="!frozen">
        <button class="btn-ghost" @click="aiShown = !aiShown" v-text="aiShown ? 'Hide AI' : 'AI'" />
      </div>
      <div class="mr-1">
        <button
    ...
    <keep-alive ref="$tabBody">
      <vm-settings class="edit-body" v-if="nav === 'settings'" v-bind="{ readOnly, script }" />
      <vm-values class="edit-body" v-else-if="nav === 'values'" v-bind="{ readOnly, script }" />
      <vm-externals class="flex-auto" v-else-if="nav === EXTERNALS" :value="script" :updatedDep />
...
const extractLine = (str, pos, limit = Infinity) => {
  if (pos >= 0) {
    const i = str.lastIndexOf("\n", pos) + 1;
    const j = str.indexOf("\n", pos);
    const line = str.slice(i, j > 0 ? j : undefined);
...
import VmSettings from "./settings.vue";
import VMSettingsUpdate from "./settings-update.vue";
import VmValues from "./values.vue";
import VmHelp from "./help.vue";
import VmAi from "./ai.vue";
...
const navItems = computed(() => {
  ...
  return {
    ...
    ...((req || res) && { [EXTERNALS]: trueJoin.call([req, res], "/") }),
    help: "?",
  };
});
...
{
  // This scoped block keeps the initialization locals out of module scope.
...
onMounted(() => {
  ...
  if (browserWindows && options.get("editorWindow") && window.history.length === 1) {
  ...
  const hk = (hotkeys.value = [
    ...
    ...(
      Object.entries($codeComp.expandKeyMap() as Record<string, string>) as [string, string][]
...
function getCode() {
  return $codeComp?.getRealContent() || code.value;
}
function applyAiCode(newCode) {
...
function close(entirely?: boolean) {
  if (!entirely && nav.value !== "code") {
    ...
    if (IS_FIREFOX) (getActiveElement() as HTMLElement | null)?.blur();
  }
}
...
async function savePosition(wnd?: browser.windows.Window) {
...
function setupSavePosition({ id: curWndId, tabs = [] }: browser.windows.Window) {
  if (tabs.length === 1) {
    const { onBoundsChanged } = chrome.windows;
    if (onBoundsChanged) {
      // triggered on moving/resizing, Chrome 86+
      onBoundsChanged.addListener((wnd) => {
        if (wnd.id === curWndId) savePosition(wnd as unknown as browser.windows.Window);
      });
    } else {
