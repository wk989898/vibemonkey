...
type MutableScript = VMScript & Record<string, any>;

let updateThrottle: Promise<unknown> | boolean | null;
 ...
 * @param {string} [code]
 */
function initScript(script: MutableScript, sizes: number[], code?: string) {
  ...
  const descText = trueJoin.call(desc, "\n");
  ...
  script.$canUpdate =
    getScriptUpdateUrl(script) && (script.config.shouldUpdate ? 1 : -1) /* manual */;
  loadScriptIcon(script, store, true);
}

export function loadData() {
  const id = +(store.route.paths?.[1] || 0);
  return requestData(id).catch(id && (() => requestData()));
  /* Catching in order to retry without an id if the id is invalid.
   * Errors will be shown in showUnhandledError. */
}

async function requestData(id?: number) {
  const [data] = await Promise.all([
    sendCmdDirectly("GetData", { id, sizes: true }, { retry: true }),
    options.ready,
  ]);
  const { [SCRIPTS]: allScripts, sizes, ...auxData } = data as Record<string, any>;
  Object.assign(store, auxData); // initScripts needs `cache` in store
  const scripts: MutableScript[] = [];
  const removedScripts: MutableScript[] = [];
  // modifying scripts without triggering reactivity
  (allScripts as MutableScript[]).forEach((script, i) => {
...
function initMain() {
  ...
  Object.assign(handlers, {
    ...
    async UpdateScript({
      ...
      update?: Partial<MutableScript> & Record<string, any>;
      where?: { id: number };
      code?: string;
    } = {}) {
      if (!update || !where) return;
      if (
        updateThrottle ||
      ...
      const oldScript = (
        i1 >= 0 ? store.scripts[i1] : i2 >= 0 ? store.removedScripts[i2] : null
      ) as MutableScript | null;
      const script = oldScript || (update.meta && store.canRenderScripts && ({} as MutableScript)); // a new script was just saved or installed
      ...
      const [sizes] = (await sendCmdDirectly("GetSizes", [where.id])) as [number[]];
      const { search } = store;
      ...
      if (search) {
        performSearch(
          [script as unknown as Parameters<typeof performSearch>[0][number]],
          search.rules,
        );
      }
      if (removed != null) {
      ...
      if (store.tags && (removed != null || (oldTags || "") !== (script.custom.tags || "")))
        updateTags();
    },
    RemoveScripts(ids) {
      store.removedScripts = store.removedScripts.filter(
        (script) => !ids.includes(script.props.id),
      );
    },
  });
}
