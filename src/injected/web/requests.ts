...
addHandlers({
  ...
  HttpRequested(msg) {
    ...
    if (hasOwnProperty(msg, ERROR)) {
      ...
      if (cb) cb(msg);
      else log(ERROR, displayNames[req.scriptId], msg);
      return;
    }
    if (!cb) {
      return;
    ...
    const { data } = msg;
    const { [kResponse]: response, [kResponseHeaders]: headers } = data;
    let raw = req[kRaw];
    if (response != null) {
      if (req[kXhrType]) {
        req[kRaw] = response;
    ...
    if (req[kXhrType]) {
      ...
      setOwnProp(
        data,
        kResponseText,
        safeBind(parseRaw, data, req, msg, kResponseText),
        true,
        "get",
      );
    }
...
function parseRaw(req, msg, propName) {
  ...
  if (kRaw in req) {
    ...
    if (!req[kXhrType]) {
      tmp = res;
      res = "";
      safeCall(forEach, tmp, (chunk) => (res += chunk));
      req[kRaw] = [res];
      req[kResponseText] = res;
    }
    if (
      (responseType === kDocument && (ct = kContentTextHtml)) ||
      (!responseType &&
        propName === kResponseXML &&
        safeCall(indexOf, PARSEABLE_TYPES, (ct = getContentType(msg) || kContentTextHtml)) >= 0) ||
      responseType === "json"
    ) {
      try {
        res = ct ? safeCall(parseFromString, new SafeDOMParser(), res, ct) : jsonParse(res);
      } catch (e) {
        res = null; /* per specification */
      }
    }
...
export function onRequestCreate(opts, context, fileName = "") {
  ...
  const { withCredentials = true, anonymous = !withCredentials, [UPLOAD]: upload } = opts;
  // setting opts.onload and onerror before EVENTS_TO_NOTIFY
  if (context.async)
    res = new SafePromise((resolve, reject) => {
  ...
  idMap[id] = req;
  const firefoxVersion = typeof IS_FIREFOX === "number" ? IS_FIREFOX : 0;
  data = (data == null && []) ||
    ...
    (firefoxVersion >= 56 && [data]) || [data, "bin"];
...
export function onRequestInitError(opts: { onerror?: unknown }, err: unknown) {
  const onerror = opts[kOnerror];
...
function getFormData(data) {
  try {
    return [[...safeCall(formDataEntries, data)], "fd"];
  } catch (e) {
    /**/
  }
  try {
