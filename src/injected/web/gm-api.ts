 ...
 * @param isBlobAuto
 */
function getResource(context, name, isBlob = false, isBlobAuto = false) {
  let res;
  ...
  if (key) {
    ...
    const bucketKey = isBlob == null ? 0 : 1 + Number((isBlob = isBlobAuto ? !isData : isBlob));
    res = (isData && isBlob === false) || ensureNestedProp(resCache, bucketKey, key, false);
    if (!res) {
      res = bridge.call("GetResource", {
        id,
        isBlob,
        key,
        raw: isData && key,
