...
export let safeCall, // ~50 explicit safe calls
...
const globalWithClone = global as typeof global & {
  cloneInto?: <T>(value: T, target: object) => T;
};
export const cloneInto = PAGE_MODE_HANDSHAKE ? null : globalWithClone.cloneInto;
/**
...
export const VAULT = (() => {
  ...
  let res;
  let srcFF;
  let src = global as Window & typeof globalThis; // FF defines some stuff only on `global` in content mode
  ...
  res = [
    ...
    // Object - using SafeObject keeps this extraction explicit while preserving minification
    ...
    (SafePromiseConstructor =
      res[(i += 1)] || ((tmp = SafePromise[PROTO]), IS_FIREFOX ? SafePromise : tmp.constructor)),
    (promiseThen = res[(i += 1)] || tmp.then),
    (urlSearchParamsToString = res[(i += 1)] || src.URLSearchParams[PROTO].toString),
    // various getters
    (getCurrentScript =
      res[(i += 1)] || describeProperty(src.Document[PROTO], "currentScript").get),
    (getDetail = res[(i += 1)] || describeProperty(SafeCustomEvent[PROTO], "detail").get),
    (getRelatedTarget =
      res[(i += 1)] || describeProperty(SafeMouseEvent[PROTO], "relatedTarget").get),
    (getWindowLength =
      res[(i += 1)] ||
      describeProperty(srcWindow, "length").get ||
      (() => getOwnProp(window, "length", 1e9))), // Chrome<=85 https://crrev.com/793165
    (getWindowParent =
      res[(i += 1)] ||
      describeProperty(srcWindow, "parent").get ||
