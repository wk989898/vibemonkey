...
export const safeCall = safeApply.call.bind(safeApply.call); // ~75 explicit safe calls
...
export const {
  ...
  Object, // for minification and guarding bundler-generated Object(import) calls
  Promise: SafePromise,
...
export const { then: promiseThen } = SafePromise[PROTO];
export const { indexOf: stringIndexOf, slice } = "";
export const safeCharCodeAt = safeApply.call.bind("".charCodeAt); // faster than a bound string charCodeAt
...
export let topRenderMode =
  window !== top
    ? 0
    : // TODO: revisit when link-preview is shipped in Chrome
      document.prerendering && document.visibilityState === "hidden"
      ? 2
      : 1;
