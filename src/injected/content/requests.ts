...
const { fetch: safeFetch, FileReader: SafeFileReader, FormData: SafeFormData } = global;
...
onScripts.push((data) => {
  ...
  for (let p = getPrototypeOf(navigator), i = 0; p && i < UA_PROPS.length; i++) {
    ...
    if (!i && (p = describeProperty(p, "userAgentData")) && (getUAData = p.get)) {
      ...
      if (
        (p = safeCall(getUAData, navigator)) &&
        (p = getPrototypeOf(p)) &&
...
addHandlers({
  ...
  async HttpRequest(msg, realm) {
    if (IS_FIREFOX)
      msg = nullObjFrom(msg); // copying into our realm to set its props freely
    else setPrototypeOf(msg, null);
    const { url } = msg;
    const data = !IS_FIREFOX && msg.data;
    const uaData = getUAData && safeCall(getUAData, navigator);
    const sch = safeCall(slice, url, 0, 5);
    ...
    // In Firefox we recreate FormData in the background decodeBody path.
    ...
    msg.ua = safeCall(map, getUAProps, (fn, i) => safeCall(fn, !i ? navigator : uaData));
    return sendCmd("HttpRequest", msg);
  },
  AbortRequest: true,
  UA: () => safeCall(getUAProps[0], navigator),
  UAD() {
    if (getUAData) {
      const res = createNullObj();
      const uaData = safeCall(getUAData, navigator);
      for (let i = 1; i < getUAProps.length; i++) {
        res[UA_PROPS[i]] = safeCall(getUAProps[i], uaData);
      }
      return res;
    }
  },
  UAH: (hints) => safeCall(getHighEntropyValues, safeCall(getUAData, navigator), hints),
});
    ...
    if (response != null) {
      if (msg.blobbed) {
        ...
        if (isBlobXhr(req)) {
          ...
          response = safeCall(getTypedArrayBuffer, response);
        } else {
...
async function requestVirtualUrl(msg, url, isDataUri, realm) {
  ...
  const data = !isDataUri
    ? await importBlob(url, wantsBlob)
    : wantsResult || url.length > 100e3
      ? decodeResource(url, wantsBlob ? SafeBlob : SafeUint8Array)
      : url;
  if (fileName) {
...
async function importBlob(url, isBlob) {
  ...
  return isBlob ? safeCall(getBlob, response) : safeCall(getArrayBuffer, response);
}
 ...
 */
function processChunk(req, data, msg = undefined) {
  if (!req[kXhrType]) {
  ...
  if (!arr && U8_fromBase64 && len === lenAll) {
    ...
    let pos = msg?.chunk || 0;
    if (U8_fromBase64) safeCall(U8_set, arr, /**@type{Uint8Array}*/ data, pos);
...
async function encodeBody(body, mode) {
  ...
  return new SafePromise((resolve) => {
    safeCall(on, reader, LOAD, () =>
      resolve([safeCall(getReaderResult, reader), safeCall(getBlobType, blob), wasBlob]),
    );
