import { sendMessage } from "@/common";
import { U8_fromBase64 } from "../util";
export * from "./util-task";

/** When looking for documentElement, use '*' to also support XML pages
 * Note that we avoid spoofed prototype getters by using hasOwnProperty, and not using `length`
 * as it searches for ALL matching nodes when this tag wasn't cached internally. */
export const elemByTag = (tag, i = 0) =>
  getOwnProp(safeCall(getElementsByTagName, document, tag), i || 0);
const { TextDecoder: SafeTextDecoder } = global;
const { createElementNS } = document;
const tdDecode = SafeTextDecoder[PROTO].decode;
const kTextContent = "textContent";
// required in FF to circumvent CSP style-src https://bugzil.la/1706787
const setTextContent = describeProperty(Node[PROTO], kTextContent).set;
const regexpTest = RegExp[PROTO].test; // Deeply unsafe. TODO: remove.
export const { createObjectURL } = URL;

/**
 * @param {string} tag
 * @param {function} cb - callback runs immediately, unlike a chained then()
 * @param {?} [arg]
 * @returns {Promise<void>}
 */
export const onElement = (tag, cb, arg) =>
  new SafePromise((resolve) => {
    if (elemByTag(tag)) {
      resolve(cb(arg));
    } else {
      const observer = new MutationObserver(() => {
        if (elemByTag(tag)) {
          observer.disconnect();
          resolve(cb(arg));
        }
      });
      // documentElement may be replaced so we'll observe the entire document
      observer.observe(document, {
        childList: true,
        subtree: true,
      });
    }
  });
export const makeElem = (tag, attrs = undefined) => {
  const el = safeCall(createElementNS, document, "http://www.w3.org/1999/xhtml", tag);
  if (attrs && isString(attrs)) {
    safeCall(setTextContent, el, attrs);
  } else if (attrs) {
    safeCall(forEach, objectKeys(attrs), (key) => {
      if (key === kTextContent) safeCall(setTextContent, el, attrs[key]);
      else safeCall(setAttribute, el, key, attrs[key]);
    });
  }
  return el;
};
export const makeSafeBlob = (data, type = "") =>
  new SafeBlob([data], {
    __proto__: null,
    type,
  } as BlobPropertyBag);

/**
 * @param {string} raw
 * @param {true | false | typeof Blob | typeof Uint8Array } [isBlob]
 * @return {string|Uint8Array|Blob|string}
 */
export const decodeResource = (raw, isBlob) => {
  let res;
  const pos = safeCall(stringIndexOf, raw, ",");
  const mimeType = pos < 0 ? "" : safeCall(slice, raw, 0, pos);
  const mimeData = pos < 0 ? raw : safeCall(slice, raw, pos + 1);
  if (isBlob === false) {
    return `data:${mimeType};base64,${mimeData}`;
  }
  res = safeAtob(mimeData);
  // TODO: do the check in BG and cache/store the result because safe-guarding all the stuff
  // regexp picks from an instance internally is inordinately complicated
  if (safeCall(regexpTest, /[\x80-\xFF]/, res)) {
    if (U8_fromBase64) {
      res = U8_fromBase64(mimeData);
    } else {
      const len = res.length;
      const bytes = new SafeUint8Array(len);
      for (let i = 0; i < len; i += 1) {
        bytes[i] = safeCharCodeAt(res, i);
      }
      res = bytes;
    }
    if (!isBlob) res = safeCall(tdDecode, new SafeTextDecoder(), res);
  }
  if (isBlob && isBlob !== SafeUint8Array) {
    res = makeSafeBlob(res, mimeType);
    if (isBlob !== SafeBlob) res = createObjectURL(res);
  }
  return res;
};

/**
 * Used by `injected`
 * @param {string} cmd
 * @param data
 * @param {{retry?: boolean}} [options]
 * @return {Promise}
 */
export const sendCmd = (cmd, data, options = undefined) =>
  sendMessage(
    {
      cmd,
      data,
      url: location.href,
      // to replace MessageSender.url which doesn't change on soft navigation
      [kTop]: topRenderMode,
    },
    options,
  );
