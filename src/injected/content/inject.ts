...
let getAttribute;
let querySelector;
type VMInitInjectionFn =
  | ((
      isFirefox: typeof IS_FIREFOX,
      pageModeHandshake?: string,
      vaultId?: string,
    ) => (onHandle: typeof bridge.onHandle, logging: Console) => VMInvokeContentFn)
  | null;
type VMInvokeContentFn = (cmd: string, params: unknown, realm?: VMBridgeMode, node?: Node) => void;

// https://bugzil.la/1408996
let VMInitInjection = window[INIT_FUNC_NAME] as VMInitInjectionFn;
...
export function injectPageSandbox(data) {
  ...
  if (useOpener(opener) || useOpener(window !== top && parent)) {
    ...
    inject(
      {
        code: `parent["${vaultId}"] = [this, 0]` /* DANGER! See addVaultExports */,
      },
      () => {
      ...
      if (IS_FIREFOX) {
        ...
        try {
          safeCall(
            fire,
            opener,
            new SafeMouseEvent(VAULT_WRITER, {
          ...
          safeCall(
            fire,
            opener,
            new SafeCustomEvent(VAULT_WRITER, {
              detail: vaultId,
            }),
          );
        } catch (e) {
  ...
  function startHandshake() {
    ...
    inject({
      code:
        `(${VMInitInjection}(${IS_FIREFOX},'${handshakeId}','${vaultId}'))()` +
...
export async function injectScripts(data, info, isXml) {
  ...
  const moreData = (more || toContent.length) && sendFeedback(toContent, more);
  ...
  if (pageBodyScripts || contLists?.[BODY]) {
    await onElement(
      BODY,
      !wasInjectableFF || !pageBodyScripts
        ? injectAll
        : (arg) => {
...
function didPageLoseInjectability(toContent, scripts) {
  ...
  if (!toContent) {
    ...
    !(res = safeCall(querySelector, document, 'meta[http-equiv="content-security-policy" i]')) ||
    !safeCall(getAttribute, res, "content")
  ) {
...
function sendFeedback(toContent, more = undefined) {
...
function triageScript(script) {
  ...
  realm =
    (realm === AUTO && !pageInjectable) || realm === CONTENT ? CONTENT : pageInjectable && PAGE;
  if (realm) {
    const lists =
      realm === CONTENT
    ...
    setOwnProp(
      gmi,
      "scriptMetaStr",
      metaStr[0] || safeCall(slice, script.code[metaStr[1]], metaStr[2], metaStr[3]),
    );
...
function inject(item, iframeCb = undefined) {
  const { code } = item;
  ...
  const divRoot =
    injectedRoot ||
    (attachShadow
      ? safeCall(attachShadow, div, {
          mode: "closed",
        })
      : div);
  if (isCodeArray) {
    safeApply(append, script, code as ArrayLike<unknown>);
  }
  ...
  if (!injectedRoot) {
    ...
    safeCall(appendChild, elemByTag("*") || document, div);
  }
  if (onError) {
...
function injectAll(runAt) {
  ...
  for (let inPage = 1; inPage >= 0; inPage--) {
    ...
    if (items) {
      ...
      if (!inPage) safeCall(promiseThen, nextTask(), () => tardyQueueCheck(items));
...
function setupContentInvoker() {
  invokeContent = (VMInitInjection as Exclude<VMInitInjectionFn, null>)(IS_FIREFOX)(
    bridge.onHandle,
    logging,
  );
...
export function addNonceAttribute(script) {
  if (nonce) safeCall(setAttribute, script, "nonce", nonce);
}
function addVaultExports(vaultSrc) {
  ...
  safeCall(forEach, CONSOLE_METHODS, (k) => {
