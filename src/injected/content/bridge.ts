...
const handlers = createNullObj<Record<string, unknown>>();
const bgHandlers = createNullObj<Record<string, unknown>>();
export const grantless = createNullObj();
/** @type {function(VMInjection)[]} */
export const onScripts = [];
const addHandlersImpl = (
...
const bridge = {
  ...
  reify: undefined as Promise<void> | false | undefined,
  // realm is provided when called directly via invokeHost
  async onHandle(
    { cmd, data, node }: { cmd: string; data?: any; node?: Node },
    realm?: VMBridgeMode,
  ) {
    let res, err;
    ...
    try {
      ...
      res =
        handle === true
          ? sendCmd(cmd, data, undefined)
          : safeCall(handle as (...args: any[]) => unknown, node, data, realm || PAGE, nodeRet);
      if (isPromise(res)) {
        res = await res;
      }
    } catch (e) {
...
browser.runtime.onMessage.addListener(({ cmd, data }, src) => {
  const handler = bgHandlers[cmd] as
    | ((data: unknown, src: browser.runtime.MessageSender) => unknown)
    | undefined;
  if (handler) {
    data = handler(data, src);
    if (data && isPromise(data)) safeCall(promiseThen, data, null, logging.error);
    return data;
  }
});
