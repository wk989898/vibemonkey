...
// Make sure to call explicit safe methods in code that may run after CONTENT userscripts
async function init() {
  const isXml = document instanceof XMLDocument;
  const xhrData = getXhrInjection();
  const dataPromise = sendCmd(
    "GetInjected",
    {
  ...
  const data =
    xhrData ||
    (IS_FIREFOX && Event[PROTO].composedPath ? await getDataFF(dataPromise) : await dataPromise);
  ...
  if (IS_FIREFOX && info) {
    // must redefine now as it's used by injectPageSandbox
    IS_FIREFOX = parseFloat(info.ua.browserVersion);
  }
