...
export const throwIfProtoPresent =
  process.env.DEBUG &&
  ((obj) => {
 ...
 * @template T
 */
export const setOwnProp = (obj, key, value, mutable = true, valueKey = undefined) =>
...
export const ensureNestedProp = (obj, bucketId, key, defaultValue) => {
  ...
  const val = bucket[key] ?? (bucket[key] = defaultValue ?? createNullObj());
  return val;
};
...
/** Unlike a native push() call this one doesn't call possibly spoofed Array.prototype setters */
export const safePush = (arr, val) => setOwnProp(arr, arr.length, val);
