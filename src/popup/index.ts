...
import { render, showUnhandledError } from "@/common/ui";
...
void initialize().catch(showUnhandledError);
render(App);
...
async function setPopup(data, { [kFrameId]: frameId, url }) {
  ...
  if (!data[MORE]) {
    Object.assign(
      data[IDS],
      await sendCmdDirectly("GetMoreIds", {
        url,
        [kTop]: isTop,
        [IDS]: data[IDS],
      }),
  ...
  const idMap = mapEntry.call(data[IDS], null, (id, val) => val !== idMapOld[id] && id);
  ...
  if (ids.length) {
    ...
    const metas =
      data[SCRIPTS]?.filter(({ props: { id } }) => ids.includes(id)) ||
  ...
  if (!hPrev) {
    ...
    if (isFullscreenPopup && hPrev > document.body.clientHeight) onResize(undefined);
  }
}
...
async function initialize() {
  ...
  let [cached, data, [failure, reason, reason2]] = await sendCmdDirectly("InitPopup", null, {
    retry: true,
  });
  if (!reason) {
    failure = "";
