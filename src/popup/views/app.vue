  ...
  >
    <div class="flex menu-buttons">
      ...
      <span
        class="menu-area"
        :data-message="
          optionsData.isApplied ? i18n('menuScriptEnabled') : i18n('menuScriptDisabled')
        "
        :tabIndex
        @click="onToggle"
      >
    ...
    <div
      ...
      :key="scope.name"
    >
      <div class="menu-item menu-area menu-group" :tabIndex @click="toggleMenu(scope.name)">
        ...
        >
          ...
          >
            ...
            >
              ...
              <a
                v-if="item.data.grantless"
                class="tardy"
                tabindex="0"
                :title="item.data.grantless"
          ...
          <div class="submenu-buttons" v-show="showButtons(item)">
            ...
            <div class="submenu-button" :tabIndex :_item.prop="item" @click="showExtras">
              <icon name="more" />
            </div>
          </div>
          <div v-if="item.excludes" class="excludes-menu mb-1c mr-1c">
            ...
            <details class="mb-1">
              ...
              <small
                ...
                }}<a v-text="i18n('labelExcludeMatch')" target="_blank" :href="VM_DOCS_MATCHING" />
              </small>
            </details>
          </div>
    ...
    <div class="incognito" v-if="store.tab?.incognito" v-text="i18n('msgIncognitoChanges')" />
    <footer>
    ...
    <div v-show="topExtras" ref="$topExtras" class="extras-menu">
      ...
      <div
        v-text="i18n('updateListedCmd', `${Object.keys(store.updatableScripts).length}`)"
    ...
    <div v-if="extras" ref="$extras" class="extras-menu">
      ...
      <div
        v-text="extras.data.config.removed ? i18n('buttonRestore') : i18n('buttonRemove')"
        tabindex="0"
...
type PopupExcludeList = [string, Record<string, string>];
...
type PopupMenuItem = {
  ...
  data: VMScript;
  key: string;
  excludes: PopupExcludeList | null;
  el?: HTMLElement | null;
  upd?: string | null;
  updError?: string;
};
...
type PopupExtrasState = PopupMenuItem | PopupTopExtras;
...
type PopupScopeSeed = [string, string] | [string, string, boolean | null];

type PopupTargetElement = HTMLElement & {
  _item?: PopupExtrasState;
  cmd?: [number, string, boolean?];
};

type PopupNavigableElement = HTMLElement & {
  rect: DOMRect;
};

type PopupDomainInfo = {
  host?: string;
  domain?: string;
};
...
let mousedownElement: EventTarget | null = null;
let focusBug = false;
...
const needsReload = reactive<Record<number, boolean>>({});
const collator = getSortCollator();
const $extras = ref<HTMLDivElement | null>(null);
const $topExtras = ref<HTMLDivElement | null>(null);
const optionsData = reactive(
  objectPick(optionsDefaults, [IS_APPLIED, kFiltersPopup, kPopupWidth, kUpdateEnabledScriptsOnly]),
);
const activeMenu = ref("scripts");
const showSettings = ref(false);
const extras = ref<PopupMenuItem | null>(null);
const focusedItem = ref<PopupMenuItem | null>(null);
const message = ref("");
const note = ref("");
const topExtras = ref<PopupTopExtras | null>(null);

const activeLinks = computed<ActiveLink[]>(makeActiveLinks);
const injectionScopes = computed<PopupScope[]>(makeInjectionScopes);
const findUrls = computed<Record<string, string>>(makeFindUrls);
const reloadHint = computed<string | false>(makeReloadHint);
...
Object.assign(handlers, {
  async UpdateScript(payload: PopupUpdateMessage = {}) {
...
function compareByCoord({ rect: a }: PopupNavigableElement, { rect: b }: PopupNavigableElement) {
  return a.top - b.top || a.left - b.left;
}
function reloadTab() {
  return browser.tabs.reload(store.tab.id);
}
function makeActiveLinks(): ActiveLink[] {
  ...
  return [support && [support, i18n("menuFeedback")], home && [home, i18n("buttonHome")]].filter(
    (item): item is ActiveLink => !!item,
  );
}
function makeInjectionScopes(): PopupScope[] {
  ...
  let updatableScripts: Record<number, PopupMenuItem> | undefined;
  const scopes: PopupScopeSeed[] = [];
  if (injectable) {
    scopes.push(["scripts", i18n("menuMatchedScripts"), groupDisabled || null]);
  }
  if (injectable && groupDisabled) {
    scopes.push(["disabled", i18n("menuMatchedDisabledScripts"), false]);
  }
  scopes.push(["frameScripts", i18n("menuMatchedFrameScripts")]);
  return scopes
    .map(([name, title, groupByEnabled]) => {
      let scripts = (store[name] || store.scripts) as VMScript[];
      if (groupByEnabled != null) {
        scripts = scripts.filter((script) => !script.config.enabled === !groupByEnabled);
      }
      const numTotal = scripts.length;
      const numEnabled =
        groupByEnabled == null
          ? scripts.reduce((num, script) => num + script.config.enabled, 0)
          : numTotal;
      if (hideDisabled === "hide" || hideDisabled === true) {
        scripts = scripts.filter((script) => script.config.enabled);
      }
      const list = scripts
        .map((script) => {
    ...
    .filter((scope): scope is PopupScope => !!scope);
}
function makeFindUrls(): Record<string, string> {
...
function makeReloadHint(): string | false {
  return (
...
async function showExtras(evt: MouseEvent) {
  const el = evt.currentTarget as PopupTargetElement | null; // get element with @click, not the inner stuff like icon
  const item = el?._item;
  if (!el || !item) return;
  const isPerItem = "data" in item;
  const what = isPerItem ? extras : topExtras;
  if (!what.value) {
    evt.stopPropagation(); // prevent app's @click from resetting extras and topExtras
    what.value = item;
    item.el = el.closest<HTMLElement>(SCRIPT_CLS) || el;
    await nextTick();
...
function onManage(evt: number | MouseEvent) {
  sendCmdDirectly(
    "OpenDashboard",
    evt === 1 || (evt instanceof MouseEvent && (evt.button === 1 || evt.ctrlKey))
      ? TAB_SETTINGS
      : "",
  ).then(close);
}
function onOpenUrl(e: MouseEvent) {
  const target = e.target;
  const el =
    target instanceof Element ? target.closest<HTMLAnchorElement>("a[href][target=_blank]") : null;
  if (!el) return;
  e.preventDefault();
  sendCmdDirectly("TabOpen", { url: el.href }).then(close);
}
function onEditScript(item: PopupMenuItem | null) {
  if (!item) return;
  sendCmdDirectly("OpenEditor", item.data.props.id).then(close);
}
function onCommand(evt: MouseEvent | KeyboardEvent) {
  const { type, currentTarget: el } = evt;
  const element = el as PopupTargetElement | null;
  if (!element?.cmd) return;
  if (type === "mousedown") {
    mousedownElement = element;
    evt.preventDefault();
  } else if (type === "keydown" || mousedownElement === element) {
    const [id, key, autoClose = true] = element.cmd;
    const idMap = store.idMap;
    const frameId = Number(Object.keys(idMap).find((frameIdStr) => id in idMap[frameIdStr]));
    sendTabCmd(
      store.tab.id,
      "Command",
      {
      ...
      { [kFrameId]: frameId },
    ).then(autoClose ? close : undefined);
  }
}
function onToggleScript(item: PopupMenuItem) {
  const { data } = item;
  const enabled: NumBool = data.config.enabled ? 0 : 1;
  const { id } = data.props;
...
function onCreateScript() {
  sendCmdDirectly("OpenEditor", null).then(close);
}
function onOpenPageAi() {
  sendTabCmd(store.tab.id, "AiOpenPanel", null, { [kFrameId]: 0 }).then(close);
}
...
function onRemoveScript() {
  ...
  const removed: NumBool = config.removed ? 0 : 1;
...
async function onExclude() {
  ...
  const { host, domain } = (await sendCmdDirectly("GetTabDomain", url)) as PopupDomainInfo;
  item.excludes = [`${url.split("#")[0]}*`, { host, group: `*.${domain}` }];
  await makePause(); // $nextTick runs too early
  item.el?.querySelector<HTMLInputElement>("input")?.focus(); // not using $refs as multiple items may show inputs
}
function onExcludeClose(item: PopupMenuItem) {
  item.excludes = null;
  focus(item);
}
async function onExcludeSave(item: PopupMenuItem, btn?: string) {
...
function navigate(dir: string) {
  const elems: PopupNavigableElement[] = [];
  for (const el of document.querySelectorAll<PopupNavigableElement>('[tabindex="0"]')) {
  ...
  let index = elems.indexOf(getActiveElement() as PopupNavigableElement);
  let found;
  if (index < 0) {
    index = 0;
  } else {
...
function focus(item: PopupExtrasState | null) {
  const root = item?.el;
  if (root) {
    (root.querySelector<HTMLElement>(".menu-area") || root).focus();
  }
}
function delegateMouseEnter({ target }: MouseEvent) {
  const el = target as HTMLElement | null;
  if (!el) return;
  if (el.tabIndex >= 0) el.focus();
  else if (!el.closest("[data-message]")) message.value = "";
}
function delegateMouseLeave({ target }: MouseEvent) {
  const el = target as HTMLElement | null;
  if (el === getActiveElement() && !isInput(el)) el.blur();
}
function updateMessage() {
  const activeElement = getActiveElement() as HTMLElement | null;
  message.value = activeElement?.dataset.message || "";
}
function showButtons(item: PopupMenuItem) {
...
onMounted(() => {
  ...
  for (const key of ["up", "down", "left", "right"]) {
    keyboardService.register(key, navigate.bind(null, key[0]), { condition: "!inputFocus" });
  }
