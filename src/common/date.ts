...
const getDayOfYear = (date: Date) =>
  Math.floor((date.getTime() - new Date(getYear(date), 0, 1).getTime()) / DAY_MS) + 1;
const getWeekOfYear = (date: Date) =>
  Math.floor((date.getTime() - new Date(getYear(date), 0, 1).getTime()) / WEEK_MS) + 1;
const toLocaleString = (date: Date, opts: Intl.DateTimeFormatOptions) =>
  date.toLocaleString([navigator.language], opts);
...
} satisfies Record<string, (date: Date) => string | number>;

let re: RegExp | undefined;

export function formatDate(tpl: string, date = new Date()) {
  if (!re) {
    re = new RegExp(
      `${
  ...
  return tpl.replace(re, (s, literal: string | undefined) =>
    hasOwnProperty(DATE_FMT, s) ? `${DATE_FMT[s](date)}` : (literal ?? s),
  );
}
