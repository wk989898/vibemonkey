...
if (process.env.DEV && process.env.IS_INJECTED !== "injected-web") {
  const get = () => {
    throw (
      "Do not use `for-of` with Map/Set. Use forEach or for-of with a [...copy]" +
      "\n(iterator semantics in these objects are intentionally avoided here)."
    );
  };
...
const PORT_ERROR_RE =
  /(Receiving end does not exist)|The message port closed before|moved into back\/forward cache|$/;

export function initHooks<T extends unknown[] = unknown[]>() {
  const hooks = new Set<(...data: T) => void>();
  return {
    hook(cb: (...data: T) => void) {
      hooks.add(cb);
 ...
 * @return {Promise}
 */
export function sendCmd(cmd: string, data: unknown, options: { retry?: boolean } = undefined) {
...
export const getBgPage = () => !MV3 && browser.extension.getBackgroundPage?.();
 ...
 */
export function sendCmdDirectly(
  cmd: string,
  data: unknown,
  options: { retry?: boolean } = undefined,
  fakeSrc: Record<string, unknown> | undefined = undefined,
) {
 ...
 * @return {Promise}
 */
export function sendTabCmd(
  tabId: number,
  cmd: string,
  data: unknown,
  options: VMMessageTargetFrame = undefined,
) {
  return browser.tabs.sendMessage(tabId, { cmd, data }, options as any).catch(ignoreNoReceiver);
}

// Used by `injected`
export function sendMessage(payload: unknown, options: { retry?: boolean } = {}) {
  const { retry } = options;
 ...
 */
export async function sendMessageRetry(payload: unknown, maxDuration = 10e3) {
...
export function ignoreNoReceiver(err: unknown) {
...
export function makePause(ms = 0) {
