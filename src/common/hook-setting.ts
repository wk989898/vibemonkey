...
type SettingHook = (value: unknown) => void;

const hooks: Record<string, SettingHook[]> = {};

options.hook((data) => {
  forEachEntry.call(hooks, ([key, list]) => {
    if (list) {
...
export default function hookSetting(key: string, update: SettingHook) {
