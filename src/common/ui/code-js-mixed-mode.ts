...
type StringStreamLike = {
  current: () => string;
  backUp: (count: number) => void;
  next: () => string | undefined;
  match: (pattern: RegExp | string, consume?: boolean) => RegExpMatchArray | boolean | null;
  pos: number;
  start: number;
  string: string;
};
...
type CodeMirrorMode = {
  name?: string;
  token: (stream: StringStreamLike, state: Record<string, any>) => TokenStyle;
  startState: () => Record<string, any>;
  copyState?: (state: Record<string, any>) => Record<string, any>;
  indent?: (state: Record<string, any>, textAfter: string, line?: string) => number;
  [key: string]: any;
};

type MixedModeState = Record<string, any>;

type RuleMatchContext = {
  jsTokenStyle: TokenStyle;
  state: MixedModeState;
  stream: StringStreamLike;
  style: TokenStyle;
...
type RuleLambda = (ctx: RuleMatchContext) => boolean | void;
...
type Rule = {
  ...
  style?: string;
  mode?: CodeMirrorMode;
  hasBeginBacktick?: boolean;
  match?: RuleLambda | string | RegExp;
  onMatch?: RuleLambda;
  onMiss?: RuleLambda;
};
...
CodeMirror.defineMode(
  ...
  (config) => {
    ...
    const cssMode = CodeMirror.getMode(config, { name: "css" }) as CodeMirrorMode;

    // use htmlmixed to support highlighting css in <style> (and to a lesser extent, js in <script>)
    const htmlmixedMode = CodeMirror.getMode(config, { name: "htmlmixed" }) as CodeMirrorMode;
    ...
    const [forceHtmlModeToAttrContinuedState, htmlStateForAttrValue] = (() => {
      ...
      return [_forceHtmlModeToAttrContinuedState, stateForAttrValue];
    })();

    // Holds input parameters and return values for a rule execution
    class RunContext implements RuleMatchContext {
      jsTokenStyle: TokenStyle = null;
      state!: MixedModeState;
      stream!: StringStreamLike;
      ...
      get type(): string | undefined {
        return this.state[kJsState]?.lastType as string | undefined;
      }

      get text(): string {
        if (this.#text == null) {
          this.#text = this.stream.current();
        }
        return this.#text;
      }
    ...
    function tokenIndexOfUnescaped(stream, searchValue) {
      ...
      while (searchStartIdx < searchEndIdxExclusive) {
        ...
        if (
          candidate === tokenStartIdx || // candidate is the start of the token, it is not escaped
    ...
    function prepReparseStringTemplateInLocalMode({ stream, state }) {
      ...
      stream.backUp(stream.pos - stream.start - Number(this.hasBeginBacktick !== false));
     ...
     * @return {Rule[]}
     */
    function makeRules(prefixedRuleGroups: Record<string, Rule[]>): void {
      Object.entries(prefixedRuleGroups).forEach(([seqPrefix, rules]: [string, Rule[]]) => {
    ...
    function matchRule(
      ...
      const rules = id
        ? rulesById[id]
        : jsTokenStyle === "comment" && runCtx.text[1] === "*"
          ? rulesByLangCmt
          : rulesByType[runCtx.type];
      if (rules) {
        for (const rule of rules) {
          if (rule.id === id) {
            const ruleMatch = rule.match as RuleLambda;
            if (ruleMatch(runCtx)) {
    ...
    makeRules({
      ...
      html3: [
        ...
        {
          match: (ctx) =>
            ctx.stream.start >= ctx.state[kLocalState][kLocalHtmlPlainStringEndPos] - 1,
          next: null, // then exit local html mode
