...
export const killTrailingSpaces = (cm: any, placeholders: Array<{ body: string }>) => {
  if (!cm.options[KILL_OPT]) {
    return cm.getValue();
  }
  const cursorLines = cm.doc.sel.ranges.map((r: any) => r.head.line);
  let res = ""; // progressive concatenation is efficient in modern browsers
  let line = 0;
  cm.operation(() => {
    cm.eachLine(({ text }: { text: string }) => {
      ...
      if (len1 !== len2 && !cursorLines.includes(line)) {
        cm.replaceRange("", { line, ch: len1 }, { line, ch: len2 }, `*${KILL_OPT}`); // `*` reuses the same undo record for performance
      }
      line += 1;
    });
  });
...
CodeMirror.defineOption(SHOW_OPT, DEFAULTS[SHOW_OPT], (cm: any, val: boolean, prev: any) => {
