 ...
 */
export async function loadScriptIcon(
  script: VMScript & Record<string, any>,
  store?: { cache?: Record<string, string | Promise<string | null> | null>; isHiDPI?: boolean },
  showDefault?: boolean,
) {
  let def;
 ...
 * @param {{cache?:{}}} store
 */
export async function loadCommandIcon(
  cmdOpts: Record<string, any>,
  store: { cache?: Record<string, string | Promise<string | null> | null> },
) {
  const { icon } = cmdOpts;
