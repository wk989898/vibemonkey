...
const replaceWithFullWidthForm = (s: string) =>
  String.fromCharCode(s.charCodeAt(0) - 0x20 + 0xff00);
...
export function getScriptHome(script) {
  let custom, meta;
  return (
    (custom = script.custom)[HOMEPAGE_URL] ||
...
export function getScriptName(script) {
  return (
    script.custom.name ||
    getLocaleString(script.meta, "name") ||
...
export function getScriptPrettyUrl(script: VMScript, displayName?: string) {
  return `${extensionRoot}${
    // When called from prepareScript, adding a space to group scripts in one block visually
    displayName && IS_FIREFOX ? "%20" : ""
  }${encodeURIComponent(
    (displayName || getScriptName(script)).replace(BAD_URL_CHAR, replaceWithFullWidthForm),
  )}.user.js#${script.props.id}`;
}

export function getScriptsTags(scripts: VMScript[]) {
  const uniq = new Set();
  for (const {
    custom: { tags },
  } of scripts) {
...
export function getScriptUpdateUrl(
  script: VMScript,
  {
    ...
    allowedOnly,
    enabledOnly,
  }: { all?: boolean; allowedOnly?: boolean; enabledOnly?: boolean } = {},
) {
  if ((!allowedOnly || script.config.shouldUpdate) && (!enabledOnly || script.config.enabled)) {
